...
const UserProfile: React.FC<UserProfileProps> = ({ onClose, botUsername }) => {
  ...
  useEffect(() => {
    const unsub = onSnapshot(doc(db, 'settings', 'config'), s => {
      ...
      setSettingsLoaded(true); // ✅ Settings এসে গেছে
    });
    return () => unsub();
  }, [botUsername]);
  ...
  const referralLink = React.useMemo(() => {
    ...
    const safeBotProp = (botUsername || '').replace('@','').trim();
    const bot = cs.referralBotUsername || safeBotProp;
    if (!bot) return '';
  ...
  }, [cs.referralBotUsername, cs.referralAppName, userData?.referralCode, botUsername]);
